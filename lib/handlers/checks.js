...
_checks.GET = function(data, callback){
      ...
      checkId = typeof checkId === 'string' && checkId.trim().length === appConfig.randomStringLen ? checkId.trim() : false;
      if(checkId){
            // before reading the check, authenticate by checking the token, passed in the header
            let token = data.headers.token;
            token = typeof token === 'string' && token.trim().length === appConfig.randomStringLen ? token.trim() : false;
            if(token){
