...
handlers._user.PUT = function(data, callback){
      ...
      // validate user's data, check if its applicable for data operationg
      handlers._user.validatePayloadData(data, function (err, validData) {
            if(err){
                  callback(405, {'Error' : 'Missing required fields OR invalid data'});
            }else{
                  _data.read('users', validData.phone, function (err, usersData) {
                        if(err){
                              callback(400, {'Error' : 'User not found'});
                        }else{
                              // hash the password
                              let hashedPassword = helpers.hash(validData.password);
                              ...
                              //  we are identifying user uniquely with their no's
                              _data.update('users',validData.phone, validData, function (err) {
                                    if(err)   {
